...
function foundryCli() {
  ...
  try {
    ...
    // If we are handling a completion request, silently exit
    // DEV: Writing to stderr makes the terminal output go screwy
    // DEV: Exiting with a non-zero code breaks bash completion
  ...
  // DEV: We need to pass `Path` for Windows support
  var env = xtend({}, process.env, {PATH: PATH, Path: PATH});
